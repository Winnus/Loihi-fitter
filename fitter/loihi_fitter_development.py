...
class LoihiFitter():
    # Todo:
    # - user should be able to set traces that should be optimized, might be recordings from biological neurons, need to redesign since variable is taken from trace gen....
    ...
    def fit_model(self):
        ...
        I_decay_alignment = normalize(self.trace_generator.get_synaptic_trace())
        ...
        # set all parameters
        neurons.tau_v = 0 #self.v_decay/2**12 # decay is inversed, normally it is v/(2**12/decay)
        ...
        # monitor all traces
        I_mon = StateMonitor(neurons, 'I', record=True)
        ...
        # save all traces
        self.I_traces = I_mon.I
        
        # find best match
        best_mse = inf
        best_decay = 0
        for n in np.arange(0,4097):
            mse = self.L_n(I_decay_alignment, normalize(self.I_traces[n]), 2)
            if mse < best_mse:
                best_mse = mse
                best_decay = n
    ...
    def get_fit_result(self):
        """
        Returns a dictionary with the results of the parameter fitting.
        """
