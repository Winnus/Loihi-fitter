...
class TraceGenerator():
    ...
    def generate_traces(self):
        ...
        synaptic_var     = self.variable_interpreter.synaptic_variable[0]
        synaptic_var_unit= self.variable_interpreter.synaptic_variable[1]
        ...
        indices   = [0]
        times     = [0]*ms
        input_gen_syn = SpikeGeneratorGroup(1, indices, times)
        
        # connection to input variable
        conn1 = Synapses(input_gen_syn, neuron1, on_pre=str(input_var) + '+= weight*' + str(input_var_unit))
        conn1.connect()

        # probe's
        # essential probes
        outDecay_mon    = StateMonitor(neuron0, output_var, record=0) # output variable stimulated directly
        out_var_mon     = StateMonitor(neuron1, output_var, record=0) # output variable stimulated with synaptic input
        #inDecay_in_mon  = StateMonitor(neuron1, input_var, record=0)
        synaptic_mon    = StateMonitor(neuron1, synaptic_var, record=0)
        
        # user defined probes
        user_mons = []
        for var in self.variable_interpreter.variables_to_monitor:
            user_mons.append(
                StateMonitor(neuron1, var[0], record=0)
            )
        ...
        for mon in user_mons:
            net.add(mon)
        # run
        run_output = net.run(runtime*runtime_unit, report='text')
        
        # save traces
        # essential traces
        # make sure they are flattened, not nested arrays
        self.out_stim_trace     = outDecay_mon.get_states()[output_var].flatten()
        self.in_stim_trace      = out_var_mon.get_states()[output_var].flatten()
        #self.in_stim_trace_in   = inDecay_in_mon.get_states()[input_var].flatten()
        self.synaptic_trace     = synaptic_mon.get_states()[synaptic_var].flatten()
        
        # user traces
        user_traces = {}
        user_traces_unitless = {}
        for z in zip(user_mons, self.variable_interpreter.variables_to_monitor):
            user_traces[z[1][0]] = z[0].get_states()[z[1][0]].flatten()
            user_traces_unitless[z[1][0]] = z[0].get_states()[z[1][0]].flatten()/z[1][1]
            
        self.user_traces = user_traces
        self.user_traces_unitless = user_traces_unitless
        # check for spike's
    ...
    def get_synaptic_trace(self):
        return self.synaptic_trace / self.variable_interpreter.synaptic_variable[1]
    
    def get_user_traces(self):
        return self.user_traces_unitless
